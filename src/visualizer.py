...
class Visualizer:
    def __init__(self, config, data_manager: DataManager):
        ...
        self.colors = plt.get_cmap('inferno')(np.linspace(0, 1, 10))
        self.plot_params = {
            'linewidth': 1.5,
            'markersize': 3,
            'marker': 'o',
    ...
    def plot_area_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Contour Area vs. Time", "Frame Index", "Contour Area (m^2)")
        ax.plot(frames, area_data, color=self.colors[0], **self.plot_params)
    ...
    def plot_arc_length_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Arc Length vs. Time", "Frame Index", "Arc Length (m)")
        ax.plot(frames, arc_length_data, color=self.colors[1], **self.plot_params)
    ...
    def plot_area_vec_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Area Change vs. Time", "Frame Index", "Area Change (m^2/s)")
        ax.plot(frames, area_vec_data, color=self.colors[2], **self.plot_params)
    ...
    def plot_regression_circle_radius_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Regression Circle Radius vs. Time", "Frame Index", "Radius (m)")
        ax.plot(frames, radius_data, color=self.colors[3], **self.plot_params)
    ...
    def plot_expand_vec_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Expand Velocity vs. Time", "Frame Index", "Velocity (m/s)")
    ...
    def plot_expand_dist_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Expand Distance vs. Time", "Frame Index", "Distance (m)")
    ...
    def plot_tip_distance_vs_time(self, exp_id: int):
        ...
        fig, ax = self._setup_plot(f"Experiment {exp_id}: Tip Distance vs. Time", "Frame Index", "Tip Distance (m)")
        ax.plot(frames, tip_distance_data, color=self.colors[8], **self.plot_params)
    ...
    def plot_summary_for_experiment(self, exp_id: int):
        ...
        fig.suptitle(f"Experiment {exp_id}: Summary Plots", fontsize=self.font_sizes['title'] + 4, fontweight='bold')
        ...
        for conf in plot_configs:
            ...
            ax.plot(conf['frames'], plot_data, color=self.colors[conf['color_idx']], **self.plot_params)
        ...
        axs[-1].set_xlabel("Frame Index", fontsize=self.font_sizes['label'])
